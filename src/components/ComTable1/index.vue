<template>
  <div class="tabletemplate">
    <!-- 动态显示隐藏列功能 // 动态列功能是否显示 -->
    <el-dropdown v-if="selectColumns" :hide-on-click="false" @visible-change="dropdownSelectShow" class="el-dropdown-selectColumn" trigger="click">
      <span class="dropdown-button" :style="theme.sysTheme.styleLevel4">
        <i class="icon iconfont iconcaidan1" :style="theme.sysTheme.colorLevel3"></i>
      </span>
      <el-dropdown-menu slot="dropdown" style="text-align: center;">
        ...
        <div class="dropdowntable" style="text-align: left;">
          <el-dropdown-item v-for="(item, index) in tableColumns" :key="index">
            ...
            <template>
              <el-checkbox :checked="item.isShow" v-model="item.isShow" @change="checkChange(item)">{{ item.title }}</el-checkbox>
            </template>
          </el-dropdown-item>
        </div>
        <!-- <el-button
    ...
    <el-table :data="tableData" stripe v-loading="loading" highlight-current-row :header-cell-style="{
        'background-color': headerBackColor,
        color: '#333'
      }" :cell-style="{
        'background-color': '#ffffff',
        'border-color': cellBorderColor
      }" :row-style="{ border: 'none' }" show-overflow-tooltip style="width: 100%;margin-top: 0.24rem;" @selection-change="handleSelectionChange" ref="multipleTable">
      <!--选择  // 表格是够显示多选框-->
      ...
      <!--序号 自定义列序号-->
      <el-table-column v-if="indexShow" type="index" align="center" label="序号" fixed="left" :width="indexWidth">
        <template slot-scope="scope">
      ...
      <!-- 表格数据列 -->
      <!-- <div v-if="selectColumns ? column.isShow || !isshowShow : true"> -->
      <div>
        <el-table-column align="center" v-for="column in tableColumns" :key="column.key" :label="column.title" show-overflow-tooltip>
          <template slot-scope="scope">
      ...
      <!--操作 // 操作按钮是否显示 -->
      <el-table-column v-if="tableOperationShow" :width="operationColumnWidth" label="操作" align="center" fixed="right">
        <!-- 操作列按钮的个数，名称，功能都是可以自定义的 -->
        <template slot-scope="scope">
          <!-- <div class="tablecloumnview"> -->
          <span class="font-small font-color-light operationSpan" v-if="viewDetail && !dropdownBtn" @click="tableOperationBtn1Handle(scope.row)">{{ tableOperationText1 }}</span>
          <el-dropdown :hide-on-click="true" v-if="dropdownBtn">
            <span class="el-dropdown-link">
              {{ tableOperationText1 }}<i class="el-icon-arrow-down el-icon--right"></i>
            </span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(item,index) in dropdownmenu" :key="index">{{item}}</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
          <span class="font-small font-color-light operationSpan" v-if="linkLogin" @click="tableOperationBtn2Handle(scope.row)">{{ tableOperationText2 }}</span>
          <span class="font-small font-color-light operationSpan" v-if="linkLogout" @click="tableOperationBtn3Handle(scope.row)">{{ tableOperationText3 }}</span>
          <!-- </div> -->
        </template>
      </el-table-column>
    </el-table>
